...
The app is healthy — only bundle *delivery on a cold cache* fails. The big dev
...
- The `exposeLocalhost` tunnel (here Metro 8082 → external **:3000**) **drops the
  connection during a cold build** — a cold request returns *nothing* even past
...
*worse*: every fresh load is cold → `:3000` drops → "stuck loading every time".
...
1. **Pre-warm every platform bundle on server startup** (dev only). After Metro's
   ...
   This moves the slow cold compile into the background so every *device* request
