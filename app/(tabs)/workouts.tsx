...
export default function WorkoutsScreen() {
  ...
  const { completedCount, equipmentTier, completedSessions } = useAppStore();
  ...
  const timelineItems: { sessionType: SessionType; status: 'completed' | 'current' | 'upcoming'; count: number }[] = [];
  ...
  for (let i = 0; i < 9; i++) {
    ...
    const cycleCount = Math.floor(completedCount / 3);
    const sessionCompletedCount = completedSessions.filter(s => s.sessionType === sessionType).length;

    timelineItems.push({ sessionType, status, count: sessionCompletedCount });
  }
          ...
          return (
            <Animated.View key={index} entering={FadeInDown.delay(index * 50).duration(400)}>
              <View style={styles.timelineRow}>
                ...
                <Pressable
                  onPress={() => isCurrent ? handleStart(item.sessionType) : null}
                  disabled={!isCurrent}
                  style={({ pressed }) => [
                    styles.timelineCard,
                ...
                >
                  <View style={[styles.cardIcon, { backgroundColor: colors.bg }]}>
                    <Ionicons name={SESSION_ICONS[item.sessionType]} size={20} color={colors.accent} />
                  </View>
